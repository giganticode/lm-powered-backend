...
export default {
  ...
  methods: {
    show() {
      ...
      // clone colors
      this.colors = JSON.parse(JSON.stringify(this.$store.getters.colorMappingEntropy));
      this.modal.show();
    },
    close() {
      this.modal.hide();
    ...
    reset() {
      let defaultColors = JSON.parse(JSON.stringify(this.$store.getters.defaultColorMappingEntropy));
